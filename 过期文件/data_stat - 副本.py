...
class DataStat(object):
	...
	details_url,community_name FROM for_sale_property WHERE community_name LIKE '%%%s%%'"%comm
		...
		new_result['avg_area'] = int(sum(i[1] for i in self.clear_datas))/len(self.clear_datas)
		floor_datas_num = 0
		floor_datss_sum = 0
		for i in self.clear_datas:
			if i[2] > 0:
				floor_datss_sum += int(i[2])
				floor_datas_num += 1
		new_result['avg_floor'] = floor_datss_sum / floor_datas_num
		self.info = self.info + "\n平均面积为 %6.0f"%(new_result['avg_area'])
		self.info = self.info + "\n平均楼层为 %6.0f"%(new_result['avg_floor'])
	...
	def out_enquiry(self,v2,comm_name,xj):
		...
		if v2['stdev']/v2['avg']*100 >10:memo1 = memo1 + ", 但标准差系数达%.2f,数据可靠性不足"%(v2['stdev']/v2['avg']*100)
		# 	print("\n标准差系统过大，数据有效性不足")

		print(datetime.date.today(),comm_name,memo1,v2['avg_floor'],avg_t_floor,who,price,memo,year,struct,elevator)
		...
		try:
			...
			cursor.execute(sql_equiry,(datetime.date.today(),comm_name,memo1,v2['avg_floor'],"住宅",who,"林晓",price,"估价师报价",memo,avg_t_floor,year,struct,elevator))
			db.commit()
		except Exception as e:
			print("~~~ Wo，fault!!!~~~~~~")
			with open('log.txt','a+') as fout:
	...
	def query_main(self, comm, xjr = ''):
		...
		# fromdate = datetime.date.today() + datetime.timedelta(days = -30)		#取一个月内的的数据
		cursor = db.cursor()
		...
		# sql = "SELECT a.* FROM (SELECT price,area,floor_index,title,spatial_arrangement,total_floor,builded_year,advantage,total_price,\
		# 		details_url,community_name FROM for_sale_property_all FORCE INDEX (date_index) WHERE first_acquisition_time >= {0}) \
		# 		as a WHERE community_name like {1}".format(fromdate,"%"+comm.encode()+"%")

		# sql = "SELECT price,area,floor_index,title,spatial_arrangement,total_floor,builded_year,advantage,total_price,\
		# details_url,community_name FROM for_sale_property_all WHERE community_name LIKE '%%%s%%'"%comm
		...
		if len(self.datas) == 0 :
			...
			# 	info = info + "%s : %s\n"%(k,self.d[k])
			    # k是lst中的每个元素
			    # d[k]是k在lst中出现的次数
			v1,v2 = self.get_anlyse(comm,self.datas)
			info = info + self.info
			# self.out_xlsx(comm,datas)
		return info,v2
...
if __name__=="__main__":
	...
	for k in data_stat.d:
		info = info + "\n%s : %s"%(k,data_stat.d[k])

	# area = list(set(zip(*data_stat.datas)[1]))
	# area.sort()
	# for i in area:
	# 	print(i)
