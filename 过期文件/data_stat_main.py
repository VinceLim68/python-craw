...
class DataStat(object):
	...
	details_url,community_name FROM for_sale_property WHERE community_name LIKE '%%%s%%'"%comm
		...
		avg_area = int(sum(i[1] for i in self.clear_datas))/len(self.clear_datas)
		floor_datas_num = 0
		floor_datss_sum = 0
	...
	def query_main(self, comm, xjr = ''):
		...
		if len(datas) == 0 :
			...
			for k in d:
			    ...
			    # k是lst中的每个元素
			    # d[k]是k在lst中出现的次数
			v1,v2 = self.get_anlyse(comm,datas)
			info = info + self.info
			self.out_xlsx(comm,datas)
			print(info)
			if xjr:
				self.out_enquiry(v2,comm,xjr)
