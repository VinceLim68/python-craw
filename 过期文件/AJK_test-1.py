...
for title,details,comm,price in zip(titles,houses,comms,prices):
    ...
    # i = 0
    # for item in house1:
    #     # item = item.get_text().replace('<br/>','').replace('\r','').replace(' ','').replace('\n','').strip()
    #     print i,mytools.clearStr(item.get_text())
    #     i += 1
    each_data['area'] = round(float(mytools.clearStr(house[0].get_text()).split('平方'.decode('utf-8'))[0]),0) 
    each_data['spatial_arrangement'] = mytools.clearStr(house[1].get_text())
    each_data['floor_index'],each_data['total_floor'] = parse.parse_floor(house[3].get_text(),'(')
    each_data['builded_year'] = int(filter(str.isdigit,house[4].get_text().encode('utf8')))
    each_data['advantage'] = house[5].get_text() if len(house) >= 8 else ""
    # # for item in house1:
    ...
    #     if U'平方' in item and U'元' not in item and len(item) < 12:
    # #         try:
    #         each_data['area'] = round(float(item.split('平方'.decode('utf-8'))[0]),0) 
    # #         except Exception, e:
    # #             with open('logtest.txt','a+') as fout:
    # #                 fout.write('*************' + str(datetime.datetime.now()) + '*************\n')
